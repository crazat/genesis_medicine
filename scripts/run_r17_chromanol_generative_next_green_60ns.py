"""R17 generative chromanol next green-target 60 ns MD follow-up.

This extends the three non-duplicate DCT/TYR candidates that completed
run_r17_chromanol_generative_next_green_30ns.py. Aryl-halogen rows remain
photosafety-review candidates, not safety-positive topical leads.

Output:
  pilot/md_r17_chromanol_generative_next_green_60ns/summary.json
  pilot/md_r17_chromanol_generative_next_green_60ns/summary.csv
"""
...
PREV = ROOT / "pilot/md_r17_chromanol_generative_next_green_30ns/summary.json"
OUT = ROOT / "pilot/md_r17_chromanol_generative_next_green_60ns"
...
def build_jobs(base) -> list[dict[str, object]]:
    ...
    for prev in prev_rows:
        ...
        job = dict(prev)
        job["name"] = str(job["name"]).replace("__30ns", "__60ns")
...
def main() -> int:
    ...
    print("R17 generative chromanol next green-target 60 ns MD")
    if not jobs:
        print("No completed next-green 30 ns jobs found.")
        return 0
    print(
        pd.DataFrame(jobs)[
            [
    ...
    print("\nFinal R17 generative chromanol next green-target 60 ns summary:")
