"""R16 topical chromanol chlorinated pigment-target 30 ns MD.

Runs remaining chlorinated DCT/TYR positional-isomer pairs that were not
covered by the R16 priority 30 ns validation. This complements the chloro
TGFB1 positional robustness run and tests pigmentation-path stability.

Output:
  pilot/md_r16_chromanol_topical_chloro_pigment_30ns/summary.json
  pilot/md_r16_chromanol_topical_chloro_pigment_30ns/summary.csv
"""
...
OUT = ROOT / "pilot/md_r16_chromanol_topical_chloro_pigment_30ns"
...
# Already covered by priority 30 ns:
#   r16_03_dct: best DCT pair
#   r16_02_tyr: best TYR pair
ALREADY_30NS = {"r16_03_dct", "r16_02_tyr"}
TARGETS = {"dct", "tyr"}
...
def build_jobs(runner) -> list[dict[str, object]]:
    ...
    rows = df[
        df["target"].str.lower().isin(TARGETS)
        & df["analog_id"].astype(str).str.contains("Cl_pos", regex=False)
        & ~df["job_id"].isin(ALREADY_30NS)
...
def main() -> int:
    ...
    print("R16 topical chromanol chloro pigment-target 30 ns MD")
    if jobs:
        print(
            pd.DataFrame(jobs)[
    ...
    print("\nFinal R16 topical chromanol chloro pigment 30 ns summary:")
