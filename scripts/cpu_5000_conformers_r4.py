"""5000-conformer ensemble × Round 4 top 100 — heavy CPU compute."""
import sys, time
...
def main():
    ...
    print(f"5000 confs × R4 top {len(df)} mol")
    ...
    pd.DataFrame(valid).to_csv(OUT / "conformers_5000_r4.csv", index=False)
    print(f"✅ conformers_5000_r4.csv ({len(valid)} success)")
    if valid:
