"""R9 yamls — cached MSA × 30 R9 candidates."""
...
def main():
    cands = pd.read_csv(OUT / "bayesian_v5_round9_candidates.csv").head(30)
    n = 0
    for tk, up, mn in TARGETS:
        msa = MSA_DIR / mn
        seq = get_seq(up, msa)
        if not seq or not msa.exists():
            print(f"⏭ {tk} skipped"); continue
        in_dir = OUT / f"inputs_r9_{tk}"
        ...
        for i, r in cands.iterrows():
            ...
            (in_dir / f"r9_{tk}_{i}.yaml").write_text(yaml.safe_dump(cfg))
            n += 1
        print(f"✅ {tk}: 30 yamls")
    print(f"\n✅ R9 total: {n}")
    return 0
