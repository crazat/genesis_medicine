"""EMB-3 (Embelin scaffold-hop lead) paper draft.

Genesis_Medicine v3 두 번째 paper:
  Title: From skin scar to systemic anti-fibrosis — EMB-3 multi-tier fibrotic
         switch modulation with predicted IPF applicability.

데이터 소스:
  pilot/scaffold_hop/embelin/outputs/ranked.csv          (ADMET ranking)
  pilot/scaffold_hop/boltz2_validation/validation_full.csv (affinity)
  pilot/scaffold_hop/md_validation/summary.csv           (MD RMSD)
  pilot/scaffold_hop/network_validation/network_full.csv (multi-target)
  pilot/scaffold_hop/cross_disease/ranked_diseases.csv   (cross-disease)
  pilot/scaffold_hop/tgfb1_latent/latent_vs_mature.csv   (cryptic pocket)
"""
...
OUT = ROOT / "manuscript_emb3"
OUT.mkdir(parents=True, exist_ok=True)


def main() -> int:
    # 데이터 로드
    ranked = pd.read_csv(SCAFFOLD / "embelin/outputs/ranked.csv")
    valid = pd.read_csv(SCAFFOLD / "boltz2_validation/validation_full.csv")
    md = pd.read_csv(SCAFFOLD / "md_validation/summary.csv")
    network = pd.read_csv(SCAFFOLD / "network_validation/network_full.csv")
    cross = pd.read_csv(SCAFFOLD / "cross_disease/ranked_diseases.csv")
    latent = pd.read_csv(SCAFFOLD / "tgfb1_latent/latent_vs_mature.csv")

    today = _dt.date.today().isoformat()

    # EMB-3 SMILES (top1 of ranked.csv)
    emb3_row = ranked.iloc[0]
    emb3_smiles = emb3_row["smiles"]

    # MD 표
    md_table = "\n".join(
        f"| {r.compound.replace('embelin_', '').upper()} × "
        f"{r.target.upper()} | {r.rmsd_mean_A:.2f} | {r.rmsd_max_A:.2f} | "
        f"{r.rmsd_final_A:.2f} |"
    ...
    net_pivot = (network.pivot_table(index="compound", columns="target",
                                      ...
                                      aggfunc="first"))
    net_table_lines = ["| compound | " +
                       " | ".join(net_pivot.columns) + " |"]
    net_table_lines.append("|----------|" + "|".join(["---:"] *
                                                       len(net_pivot.columns))
                            + "|")
    for comp in net_pivot.index:
        row_vals = [f"{net_pivot.loc[comp, c]:.3f}"
                    if pd.notna(net_pivot.loc[comp, c]) else "—"
                    for c in net_pivot.columns]
        net_table_lines.append(f"| **{comp}** | " + " | ".join(row_vals) + " |")
    network_table = "\n".join(net_table_lines)

    # Cross-disease 표 (top 8)
    cross_top = cross.head(8)
    cross_table = "\n".join(
        f"| {r.disease_name[:50]} | {r.n_targets_hit} | {r.weighted_score:.2f} |"
        for r in cross_top.itertuples()
    )
...
"""
    ...
    out_md = OUT / "manuscript.md"
    out_md.write_text(md_text, encoding="utf-8")
    print(f"✅ {out_md}")
    print(f"   words: {len(md_text.split())}")

    # references.bib 복사 (skin_scar v2 manuscript의 것 재사용)
    ref_src = ROOT / "pilot/skin_scar/results_v2/manuscript/references.bib"
    if ref_src.exists():
        (OUT / "references.bib").write_bytes(ref_src.read_bytes())
        print(f"✅ {OUT/'references.bib'}")
    return 0
