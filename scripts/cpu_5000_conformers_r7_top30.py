"""5000-conformer ensemble on R7 top 30 — heavy CPU saturation."""
...
def main():
    df = pd.read_csv(OUT / "bayesian_v3_round7_candidates.csv").head(30)
    print(f"5000 confs × R7 top {len(df)}")
    ...
    pd.DataFrame(valid).to_csv(OUT / "conformers_5000_r7_top30.csv", index=False)
    print(f"✅ conformers_5000_r7_top30.csv ({len(valid)})")
