"""R13 yamls — cached MSA × 30 R13 candidates (R11_0 family refinement)."""
...
def main():
    cands = pd.read_csv(OUT / "bayesian_v9_round13_candidates.csv").head(30)
    n = 0
    for tk, up, mn in TARGETS:
        msa = MSA_DIR / mn
        seq = get_seq(up, msa)
        if not seq or not msa.exists(): continue
        in_dir = OUT / f"inputs_r13_{tk}"
        ...
        for i, r in cands.iterrows():
            ...
            (in_dir / f"r13_{tk}_{i}.yaml").write_text(yaml.safe_dump(cfg))
            n += 1
        print(f"✅ {tk}: 30")
    print(f"\n✅ R13 total: {n}")
