"""Bayesian v5 — R1-R8 통합 → R9 candidates."""
...
def main():
    print("=" * 72)
    print("Bayesian v5 — R1-R8 통합 (R9 candidates)")
    print("=" * 72)
    ...
    r8 = pd.read_csv(OUT / "r8_affinity_consolidated.csv")
    pieces += [r7, r8]
    print(f"  R7: {len(r7)}, R8: {len(r8)}")
    ...
    with Pool(12) as p: train_fps = p.map(fp, train_max["smi"].tolist())
    ...
    with Pool(12) as p: pool_fps = p.map(fp, pool["smiles"].tolist())
    ...
    out = OUT / "bayesian_v5_round9_candidates.csv"
