"""Bayesian AL v4 — R1-R7 통합 + R8 candidates 도출.

R7 새 420 cofold 추가 후 GP retrain → R8 top 30 candidates.
"""
...
def main():
    print("=" * 72)
    print("Bayesian v4 — R1-R7 통합 (R8 candidates 도출)")
    print("=" * 72)

    # Load all consolidated
    pieces = []
    pre_r5 = OUT / "all_boltz2_affinity_consolidated_r5.csv"
    if pre_r5.exists():
        pieces.append(pd.read_csv(pre_r5))
        print(f"  Pre-R5: {len(pieces[-1])}")

    # Add R5 phase 2 (LOX/MITF) if exists from v3
    # Add R7 (new this round)
    r7 = pd.read_csv(OUT / "r7_affinity_consolidated.csv")
    print(f"  R7 added: {len(r7)}")
    pieces.append(r7)

    cofold = pd.concat(pieces, ignore_index=True)
    print(f"  Total cofold rows: {len(cofold)}")

    # Augment compound→smiles
    ...
    out_top = OUT / "bayesian_v4_round8_candidates.csv"
    pool_v.head(200).to_csv(out_top, index=False)
    print(f"\n✅ {out_top}")
    print(f"\n[Top 10 R8 EI candidates]")
