"""5000-conformer ensemble on top integrated 45 with MMFF94s minimize.
Heavy 24-core paper-tier conformational landscape.
"""
...
def gen5000(args):
    ...
    try:
        ...
        # Boltzmann population at 300K
        kt = 0.5959    # kcal/mol
        rel = np.array(energies) - min(energies)
        weights = np.exp(-rel / kt)
        weights /= weights.sum()
        eff = (weights ** 2).sum() ** -1    # effective # conformers
        return {
            "idx": idx,
            "smi": smi,
            ...
            "n_conf_within_2kcal": int((rel < 2).sum()),
            "n_conf_within_5kcal": int((rel < 5).sum()),
...
def main():
    ...
    print(f"Conformer ensemble: 5000 confs × {len(df)} mol")
    ...
    pd.DataFrame(valid).to_csv(OUT / "conformers_5000x45.csv", index=False)
    print(f"✅ conformers_5000x45.csv ({len(valid)} success)")
    if valid:
        rng = [r["energy_range"] for r in valid]
        eff = [r["boltzmann_eff_confs"] for r in valid]
        print(f"  Energy range: {min(rng):.1f}-{max(rng):.1f} kcal/mol mean {np.mean(rng):.1f}")
