"""xtb GFN2 432-conf refine on hetero10 top9997 cohort — RESUME.

Currently xtb_npass_top9997_hetero10_refine_432conf.csv has ~80 of 985 mol done.
Completes remaining ~905 mol using subprocess pattern (safe alongside OpenMM CUDA).

Output schema preserves parent columns from 384-conf csv:
  rank, np_id, smiles, log_kp_pottsguy, logp, mw, base_gap_eV_mean,
  status, n_xtb_confs, energy_au_min, energy_kcal_min, HOMO_eV, LUMO_eV,
  gap_eV_mean, gap_eV_max
"""
...
INPUT = ROOT / "pilot/cpu_meaningful/xtb_npass_top9997_hetero10_refine_384conf.csv"
OUT = ROOT / "pilot/cpu_meaningful/xtb_npass_top9997_hetero10_refine_432conf.csv"
NUM_CONFS = 432
...
def main():
    ...
    print(f"hetero10 input: {len(df)} mol")
    done = set()
    if OUT.exists():
    ...
    for _, r in df.iterrows():
        if str(r["np_id"]) in done:
            continue
        parent = {c: r[c] for c in PARENT_COLS if c in df.columns}
        todo.append(parent)
    if not todo:
        print("nothing to do")
        return
