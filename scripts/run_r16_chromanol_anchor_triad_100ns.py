...
Runs the best TGFB1, DCT, and TYR R16 pairs to 100 ns. This is a paper-strength
extension after the 30 ns matrix, TGFB1 top-6 60 ns, and pigment representative
60 ns panels are complete.

Output:
  pilot/md_r16_chromanol_anchor_triad_100ns/summary.json
  pilot/md_r16_chromanol_anchor_triad_100ns/summary.csv
"""
...
OUT = ROOT / "pilot/md_r16_chromanol_anchor_triad_100ns"
...
def build_jobs(runner) -> list[dict[str, object]]:
    ...
    for target in TARGETS:
        ...
        jobs.append(
            {
                "name": f"{job_id}__{runner.clean_name(analog_id)}__100ns",
                "job_id": job_id,
                "analog_id": analog_id,
                "target": target,
...
def main() -> int:
    ...
    print("R16 topical chromanol anchor triad 100 ns MD")
    print(
        pd.DataFrame(jobs)[
    ...
    for job in jobs:
        if str(job["job_id"]) in done:
            ...
            continue
        try:
            result = runner.run_md(job, ns=100.0)
