...
def run_solvent_leg() -> int:
    ...
    if not (sv_parm7.exists() and sv_rst7.exists()):
        ...
        embelin_mol2 = COMPLEX_DIR / "embelin.mol2"
        embelin_frcmod = COMPLEX_DIR / "embelin.frcmod"
        if not (embelin_mol2.exists() and embelin_frcmod.exists()):
            print(f"FAIL: missing embelin.mol2 ({embelin_mol2.exists()}) or embelin.frcmod ({embelin_frcmod.exists()})")
            return 8
        leap_in = sv_dir / "tleap_solvent.in"
        leap_in.write_text(f"""
source leaprc.water.tip3p
source leaprc.gaff2
loadAmberParams {embelin_frcmod}
LIG = loadMol2 {embelin_mol2}
...
def combine() -> int:
    ...
    summary = {
        ...
        "system": "embelin (parent) + holo MMP-1 (Zn nonbonded ions234lm_126_tip3p)",
