"""
Generic ABFE Phase 5 (parameterized work-dir version of zaff_phase5_abfe_production_embelin.py).

Usage:
  python scripts/zaff_phase5_abfe_production_generic.py --work pilot/abfe_benchmark_chembl/CHEMBL415 --leg all

Expects:
  {work}/complex/{complex.parm7, complex.rst7, lig.mol2, lig.frcmod, PHASE4_OK}

Produces:
  {work}/abfe_production/{complex_leg,solvent_leg,dG_bind.json,PHASE5_OK|INCONCLUSIVE}
"""
...
def run_leg(parm: pmd.Structure, lig_atoms: list[int], leg: str, ns_prod: float, out_dir: Path) -> dict:
    ...
    n_iter_eq = int(NS_EQ * 1e6 / TIMESTEP_FS / 5000)
    ...
    sampler = multistate.ReplicaExchangeSampler(
        mcmc_moves=move, number_of_iterations=n_iter_prod, replica_mixing_scheme="swap-all")
...
def run_complex_leg(work: Path, lig_name: str) -> int:
    cx_dir = work / "complex"
    out = work / "abfe_production" / "complex_leg"
...
def run_solvent_leg(work: Path, lig_name: str) -> int:
    sv_dir = work / "abfe_production" / "solvent_leg"
...
def combine(work: Path, system_label: str) -> int:
    ...
    summary = {
        "phase": "Phase 5 ABFE production (generic)",
        "system": system_label,
        "complex_leg": cx,
        "solvent_leg": sv,
        ...
        "n_lambda": N_LAMBDA, "n_replicas": N_REPLICAS,
...
def main() -> int:
    ...
    p.add_argument("--work", required=True, help="ABFE work dir (e.g. pilot/abfe_benchmark_chembl/CHEMBL415)")
    ...
    label = args.system_label or f"{work.name} + holo MMP-1 (ZAFF)"
