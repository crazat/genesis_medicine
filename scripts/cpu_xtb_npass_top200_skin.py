"""xtb GFN2 on top 200 NPASS skin-permeable compounds — CPU heavy."""
...
def run_one(args):
    ...
    try:
        ...
        if m is None:
            return {"idx": idx, "err": "invalid"}
        m = Chem.AddHs(m)
        cids = AllChem.EmbedMultipleConfs(m, numConfs=3, randomSeed=42)
        if not cids:
            return {"idx": idx, "err": "no embed"}
        results = []
        for cid in cids:
            try:
...
def main():
    ...
    top = npass.nlargest(200, "log_kp_pottsguy").reset_index(drop=True)
    print(f"NPASS top 200 by Potts-Guy logKp: {top['log_kp_pottsguy'].min():.2f} to {top['log_kp_pottsguy'].max():.2f}")

    import tempfile
    with tempfile.TemporaryDirectory(prefix="xtb_npass_") as tmpdir:
        args = [(i, top.iloc[i]["smiles"], tmpdir) for i in range(len(top))]
        t0 = time.time()
        with Pool(16) as p:
    ...
    out_csv = OUT / "xtb_npass_top200.csv"
    out.to_csv(out_csv, index=False)
    print(f"\n✅ {out_csv} ({len(valid)}/{len(top)} valid)")
