"""xtb GFN2 on Round 4 candidates — quantum energy + dipole moment."""
import sys, time
...
def main():
    ...
    print(f"xtb GFN2 on R4 top {len(df)} mol")
    args = [(i, df.iloc[i]["smiles"]) for i in range(len(df))]
    t0 = time.time()
    with Pool(12) as p:
        results = p.map_async(xtb_quantum, args).get(timeout=1200)
    valid = [r for r in results if r and "energy_au" in r]
    pd.DataFrame(valid).to_csv(OUT / "xtb_r4_quantum.csv", index=False)
    print(f"Wall: {(time.time()-t0)/60:.1f} min")
    print(f"✅ xtb_r4_quantum.csv ({len(valid)})")
