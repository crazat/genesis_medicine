"""ABFE EMB-3 × MMP-1 — full paper-tier version.

Short version (run_emb3_abfe_openmmtools.py) 검증 완료 후 paper-tier로 확장:

설계:
  - 16 lambda windows (electrostatics 9 + sterics 8, smoother)
  - 500 iterations × 10 ps/iter = 5 ns per window
  - Total simulation: 16 × 5 ns = 80 ns
  - 1 replica per window, replica exchange Hamiltonian
  - 예상 wall: 6-8시간 RTX 5090 (GPU 70-80% util 목표)
  - Statistical uncertainty 목표: ±0.5 kcal/mol

iteration당 step수 늘려서 swap overhead 비율 줄임 (GPU util ↑).
"""
...
N_LAMBDA_WINDOWS = 16
N_ITERATIONS = 500
STEPS_PER_ITERATION = 5000   # 10 ps with 2 fs timestep
EQ_NS = 0.5   # 500 ps NPT eq before alchemical
PADDING_NM = 1.2
...
def make_lambda_states(alch_system, n_windows=N_LAMBDA_WINDOWS):
    print(f"\n[3/5] Lambda schedule ({n_windows} windows, smoother)")
...
def run_replica_exchange(thermo_states, eq_pos, eq_box):
    ...
    print(f"   total simulation: "
          f"{len(thermo_states)*N_ITERATIONS*STEPS_PER_ITERATION*0.002/1000:.1f} ns")
...
def main() -> int:
    print("=== EMB-3 × MMP-1 ABFE FULL (paper-tier) ===\n")
    t_start = time.time()
