"""R8 yamls — cached MSA × 30 R8 candidates."""
...
def main():
    cands = pd.read_csv(OUT / "bayesian_v4_round8_candidates.csv").head(30)
    n = 0
    for tk, up, mn in TARGETS:
        ...
        for i, r in cands.iterrows():
            ...
            (in_dir / f"r8_{tk}_{i}.yaml").write_text(yaml.safe_dump(cfg))
            n += 1
        print(f"✅ {tk}: 30 yamls")
    print(f"\n✅ R8 total: {n}")
    return 0
