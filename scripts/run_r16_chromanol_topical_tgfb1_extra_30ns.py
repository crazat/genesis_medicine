"""R16 chromanol topical TGFB1 extra 30 ns MD extensions.

Extends the next TGFB1 R16 topical chromanol analogs beyond the priority
r16_03_tgfb1 pair that is already covered in the priority 30 ns run.

Output:
  pilot/md_r16_chromanol_topical_tgfb1_extra_30ns/summary.json
  pilot/md_r16_chromanol_topical_tgfb1_extra_30ns/summary.csv
"""
...
OUT = ROOT / "pilot/md_r16_chromanol_topical_tgfb1_extra_30ns"
...
def build_jobs(runner) -> list[dict[str, object]]:
    ...
    top = (
        df[df["target"].str.lower().eq("tgfb1")]
        .sort_values("affinity_probability_binary", ascending=False)
    )
    top = top[~top["job_id"].isin(ALREADY_EXTENDED)].head(3)
...
def main() -> int:
    ...
    print("R16 topical chromanol TGFB1 extra 30 ns MD")
