"""R17 generative chromanol next green-target 10 ns MD screen.

This follow-up is intentionally narrow: after the first R17 DCT/TYR top-green
panel has reached 60 ns, it screens the next three non-duplicate DCT/TYR
cofold hits for 10 ns. Aryl-halogen rows remain photosafety-review candidates.

Output:
  pilot/md_r17_chromanol_generative_next_green_10ns/summary.json
  pilot/md_r17_chromanol_generative_next_green_10ns/summary.csv
"""
...
OUT = ROOT / "pilot/md_r17_chromanol_generative_next_green_10ns"
...
def select_rows(df: pd.DataFrame, already_done: set[str]) -> pd.DataFrame:
    ...
    ranked = green.sort_values(
        ["affinity_probability_binary", "local_design_priority"],
    ...
    safety_green = ranked[
        ...
        & ~ranked["job_id"].astype(str).isin(selected_ids)
    ]
    ...
    for _, row in ranked.iterrows():
        if len(selected) >= 3:
            break
        if str(row["job_id"]) not in selected_ids:
            selected.append(row)
...
def main() -> int:
    ...
    print("R17 generative chromanol next green-target 10 ns MD")
    if not jobs:
        print("No non-duplicate R17 green-target cofold jobs found.")
        return 0
    print(
        pd.DataFrame(jobs)[
            [
    ...
    print("\nFinal R17 generative chromanol next green-target 10 ns summary:")
