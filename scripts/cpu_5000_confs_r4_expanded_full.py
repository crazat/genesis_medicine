"""5000-conformer ensemble × full R4 expanded 194 mol — heavy MMFF94s."""
import sys, time
...
def gen5000(args):
    ...
    except Exception as e: return {"idx": idx, "error": str(e)[:100]}


def main():
    df = pd.read_csv(OUT / "round4_expanded.csv")
    print(f"5000 confs × R4 expanded {len(df)} mol")
    ...
    pd.DataFrame(valid).to_csv(OUT / "conformers_5000_r4_expanded.csv", index=False)
    print(f"✅ conformers_5000_r4_expanded.csv ({len(valid)})")
