"""R12_11 × 10 missing skin targets MD — 2nd PAINS-free leader universal validation.

Already done (4): MMP1, SIRT1, MITF, SREBP1
Missing (10): TGFB1, CTGF, AR, LOX, TYR, TYRP1, DCT, SRD5A1, SRD5A2, PTGS2

Sequential 10 ns × 10 ≈ 100 min GPU. Output: pilot/md_r12_11_full14
"""
...
OUT = ROOT / "pilot/md_r12_11_full14"
OUT.mkdir(parents=True, exist_ok=True)

R12_11_SMI = "COc1ccc(O)c(C=CC2COc3cc(O)ccc3C2)c1"

JOBS = [
    {"name": "tgfb1__r12_11",  "tgt": "tgfb1"},
    {"name": "ctgf__r12_11",   "tgt": "ctgf"},
    {"name": "ar__r12_11",     "tgt": "ar"},
    {"name": "lox__r12_11",    "tgt": "lox"},
    {"name": "tyr__r12_11",    "tgt": "tyr"},
    {"name": "tyrp1__r12_11",  "tgt": "tyrp1"},
    {"name": "dct__r12_11",    "tgt": "dct"},
    {"name": "srd5a1__r12_11", "tgt": "srd5a1"},
    {"name": "srd5a2__r12_11", "tgt": "srd5a2"},
    {"name": "ptgs2__r12_11",  "tgt": "ptgs2"},
]
...
def run_md(name, cif, smiles, ns=10.0):
    ...
    try:
        ...
        # Extended min + slow heat to avoid NaN (lesson from r12_4 ar retry)
...
def main():
    print("R12_11 (PAINS-free methoxy variant) × 10 missing skin targets")
    results = []
    for job in JOBS:
        cif = (f"pilot/cpu_meaningful/output_r12_{job['tgt']}/boltz_results_inputs_r12_{job['tgt']}/"
               f"predictions/r12_{job['tgt']}_11/r12_{job['tgt']}_11_model_0.cif")
        r = run_md(job["name"], cif, R12_11_SMI, ns=10.0)
        results.append(r)
