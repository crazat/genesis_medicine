"""R12_23 × 11 missing skin targets MD — 3rd PAINS-free leader universal validation.

Already done (3): MITF, AR, DCT
Missing (11): MMP1, SIRT1, SREBP1, TGFB1, CTGF, LOX, TYR, TYRP1, SRD5A1, SRD5A2, PTGS2

Sequential 10 ns × 11 ≈ 100-120 min GPU. Output: pilot/md_r12_23_full14
"""
...
OUT = ROOT / "pilot/md_r12_23_full14"
OUT.mkdir(parents=True, exist_ok=True)

R12_23_SMI = "COC(=O)C1Cc2c(O)cc(O)cc2OC1C1COc2cc(O)ccc2C1"

JOBS = [
    {"name": "mmp1__r12_23",   "tgt": "mmp1"},
    {"name": "sirt1__r12_23",  "tgt": "sirt1"},
    {"name": "srebp1__r12_23", "tgt": "srebp1"},
    {"name": "tgfb1__r12_23",  "tgt": "tgfb1"},
    {"name": "ctgf__r12_23",   "tgt": "ctgf"},
    {"name": "lox__r12_23",    "tgt": "lox"},
    {"name": "tyr__r12_23",    "tgt": "tyr"},
    {"name": "tyrp1__r12_23",  "tgt": "tyrp1"},
    {"name": "srd5a1__r12_23", "tgt": "srd5a1"},
    {"name": "srd5a2__r12_23", "tgt": "srd5a2"},
    {"name": "ptgs2__r12_23",  "tgt": "ptgs2"},
]
...
def run_md(name, cif, smiles, ns=10.0):
    ...
    try:
        ...
        # Extended min + slow heat (lesson from r12_4 ar retry)
...
def main():
    print("R12_23 (PAINS-free methyl ester variant) × 11 missing skin targets")
    results = []
    for job in JOBS:
        cif = (f"pilot/cpu_meaningful/output_r12_{job['tgt']}/boltz_results_inputs_r12_{job['tgt']}/"
               f"predictions/r12_{job['tgt']}_23/r12_{job['tgt']}_23_model_0.cif")
        r = run_md(job["name"], cif, R12_23_SMI, ns=10.0)
        results.append(r)
