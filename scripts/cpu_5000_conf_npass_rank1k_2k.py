"""5000-conformer NPASS rank 1000-2000 (boltz 친화적, single-thread per mol)."""
...
def main():
    ...
    target = sorted_df.iloc[1000:2000]   # rank 1000-2000
    print(f"NPASS rank 1000-2000: {len(target)} mol")

    # Smaller pool size to leave CPU for boltz
    args = [(i + 1000, target.iloc[i]["smiles"]) for i in range(len(target))]
    t0 = time.time()
    with Pool(8) as p:   # Pool=8 (boltz 친화적, leave ~16 cores for boltz)
        results = p.map(gen5000, args)
    print(f"Wall: {(time.time()-t0)/60:.1f} min")
    valid = [r for r in results if r and "boltzmann_eff_confs" in r]
    pd.DataFrame(valid).to_csv(OUT / "conformers_2000_npass_rank1k_2k.csv", index=False)
    print(f"✅ ({len(valid)})")
