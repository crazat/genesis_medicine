"""R17 generative chromanol top green-target 60 ns MD follow-up.

This extends the same three DCT/TYR candidates selected by
run_r17_chromanol_generative_top_green_10ns.py after the 30 ns screen is
...
Output:
  ...
  pilot/md_r17_chromanol_generative_top_green_60ns/summary.csv
"""
...
OUT = ROOT / "pilot/md_r17_chromanol_generative_top_green_60ns"
...
def main() -> int:
    ...
    print("R17 generative chromanol top green-target 60 ns MD")
    if not jobs:
    ...
    for job in jobs:
        if str(job["job_id"]) in done:
            ...
            continue
        job = dict(job)
        job["name"] = str(job["name"]).replace("__10ns", "__60ns")
        try:
    ...
    print("\nFinal R17 generative chromanol top green-target 60 ns summary:")
