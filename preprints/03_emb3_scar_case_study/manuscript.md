...
Open Targets [23] disease-target mappings were used to evaluate cross-disease applicability of an EMB-3-engaging multi-target signature. For idiopathic pulmonary fibrosis (IPF), 6 of the 7 principal targets (TGF-β1, MMP-1, CTGF, SMAD3, PDGFRB, LOX) overlap directly with the EMB-3 affinity profile. For systemic scleroderma, all 7 overlap. We frame this as a **cross-disease hypothesis**: a topical EMB-3 designed primarily for skin-scar revision may have rationale for systemic anti-fibrotic application after appropriate reformulation; this is explicitly a hypothesis, not a recommendation, and the systemic application is gated on substantial wet-lab and ADMET work that is not in the present preprint.
...
The 6/7 target overlap with IPF and 7/7 with systemic scleroderma is mechanistically reasonable: the TGF-β1/Smad/MMP/CTGF axis is shared across organ-specific fibrosis [3]. We refrain from making any clinical claim. The forward question is whether systemic delivery of EMB-3 (or an EMB-3 prodrug optimized for oral or inhaled bioavailability) would replicate the in silico signal in vivo. That question is gated on (i) experimental synthesis, (ii) PK/PD studies, (iii) IRB-approved animal models, and (iv) regulatory consultation under the Korean Ministry of Food and Drug Safety pathway.
