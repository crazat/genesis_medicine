...
ZAFF-AMBER + GAFF-2 + AM1-BCC ABFE protocol on seven ChEMBL MMP-1
inhibitors spanning sub-nanomolar to micromolar potency
(\SIrange{4}{18000}{\nano\Molar}, four scaffold classes), each computed
...
agree to within \SI{0.12}{\kcalmol} for some compounds yet are systematically
biased by 8--14 \unit{\kcalmol} from the experimental affinity, while
...
Table~\ref{tab:abfe} reports the binding free energy estimate,
within-run statistical error, between-replicate dispersion
$\Delta_{\text{rep}}$, replicate-mean $\langle\dGbind\rangle$, and
deviation from experiment for the seven compounds.
...
CHEMBL443684 (Marimastat)    &     5 & $-11.8$ & $-2.9 \pm 0.7$  & ---            & ---             & $-2.9$  & $+8.9$ \\
CHEMBL94487 (RS-130830)      &    12 & $-11.2$ & $-20.9 \pm 0.6$ & $-21.0 \pm 0.6$ & ---            & $-21.0$ & $-9.7$ \\
CHEMBL257077 (prinomastat-l) &    15 & $-11.1$ & $+8.6 \pm 0.6$  & $+3.2 \pm 0.8$  & ---            & $+5.9$  & $+17.0$ \\
CHEMBL301236 (fluoro-aryl)   &    42 & $-10.5$ & $-7.5 \pm 0.4$  & $-11.2 \pm 0.7$ & ---            & $-9.4$  & $+1.1$ \\
CHEMBL292707 (Ilomastat)     &   200 & $-9.5$  & $+4.9 \pm 0.7$  & $+4.6 \pm 0.5$  & ---            & $+4.8$  & $+14.3$ \\
CHEMBL2105729                & 18000 & $-6.7$  & $+0.4 \pm 0.8$  & $+7.2 \pm 0.6$  & $+4.7 \pm 0.5$ & $+4.1$  & $+10.8$ \\
\bottomrule
\end{tabular}
\end{table}

The experimental ordering of the seven compounds spans
$\dGexp = -11.9$ to $-6.7$ \unit{\kcalmol} (i.e.\ a 5.2-\unit{\kcalmol}
window). The replicate-mean ABFE estimates span $-21.0$ to $+5.9$
\unit{\kcalmol} (a 27-\unit{\kcalmol} window) --- an unphysically wide
spread relative to the underlying chemistry. Pearson and Spearman
correlations between $\langle\dGbind\rangle$ and $\dGexp$ on this seven
compound set are uninformative: the predictor produces sign-flipped
values for three of seven compounds.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{CHEMBL94487} (RS-130830) is the most reproducible
    compound: $\Delta_{\text{rep}} = 0.12$ \unit{\kcalmol} between rep1
    ($-20.9$) and rep2 ($-21.0$). Yet the replicate mean is
    9.7~\unit{\kcalmol} more negative than experimental.
  ...
  \item Compounds with intermediate $\Delta_{\text{rep}}$
    (3--5 \unit{\kcalmol}) are split between accurate (CHEMBL301236)
    and sign-flip-failing (CHEMBL257077).
\end{itemize}
...
\todo{insert xtb GFN1 vs GFN2 cross-method finding from
\texttt{xtb\_npass\_top9997\_hetero10\_gfn1\_432conf.csv} vs the
GFN2 432-conf cohort. Spearman $\rho = 0.978$, top-10 IoU 9/10,
top-50 IoU 45/50, $n = 986$ molecules. Solvent variation matrix
(water/methanol/DMSO/hexane, ALPB) currently in progress;
expected delivery 2026-05-06.}

The xtb-based ranking on a 986-molecule screening corpus shows
cross-method Spearman correlation $\rho = 0.978$ between GFN1-xTB and
GFN2-xTB, contrasting sharply with the ABFE replicate-mean spread
documented above. We interpret this as evidence that semi-empirical QM
ranking is method-robust at the population level, while ABFE absolute
binding affinity is not robust at the single-compound level on the
ZAFF-AMBER MMP-1 system class.
...
The benchmark above is harsh: 3 of 7 compounds sign-flip their
...
Nevertheless, two findings are usable:
