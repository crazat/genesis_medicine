% # SUPERSEDED (2026-07-23) — replaced by manuscript_v1_realpanel_accuracy.md
%
% This .tex is the pre-correction draft, retained for history only; it must not be submitted or deposited.
% Its ligand panel and every IC50 / accuracy claim came from data/chembl_mmp1_calibration.csv, a fabricated
% file that a primary-source audit refuted on 2026-07-16 (named entries were the wrong molecules; 14 of its
% 15 structures were unknown to PubChem; the file had no generating script and no retrieval record). See
% preprints/_metadata/FABRICATED_PANEL_SCOPE_2026_07_16.md and
% scripts/round27_paperA/verify_panel_identity_pubchem.py.
%
% The accuracy axis has since been re-derived on a real, provenance-tracked panel (14 PubChem MMP-1
% inhibitors, IC50 0.78 nM to 98 uM, each row carrying its PubChem CID, assay AID and retrieval timestamp;
% data/mmp1_panel_pubchem.csv, pilot/abfe_realpanel_mmp1/) and rewritten as
% manuscript_v1_realpanel_accuracy.md (+ .pdf): an accuracy-limitations paper reporting systematic,
% warhead-driven ZAFF-AMBER over-binding (all 14 compounds over-bind, no sign-flips, Spearman 0.587
% with 95% CI [0.041, 0.847]). That Markdown version is the current manuscript of record.
...
\section*{Note on ligand-panel provenance and the accuracy axis (added 2026-07-18)}

The ligand panel used in this draft is drawn from a calibration file
(\texttt{data/chembl\_mmp1\_calibration.csv}) whose annotations a primary-source audit refuted on
2026-07-16. A structure-first lookup against PubChem found that every one of the seven entries naming a
specific drug is a different molecule than the drug it names, and that fourteen of the fifteen structures
are unknown to PubChem entirely; the file carries no generating script and no retrieval record. The
molecular structures themselves parse and sanitise under RDKit and carry zinc-binding chemotypes, and it
is only those structures that this work uses. Every compound name, potency value and literature
attribution in the panel file is therefore unverified and is not relied upon.

This has a consequence specific to this draft. Its accuracy axis --- the comparison of computed ABFE free
energies against an experimental affinity ($\dGexp = RT \ln \mathrm{IC}_{50}$), and every claim derived
from it (systematic bias from experiment, sign-flips, potency-class stratification, and the
recovery-of-experiment statements in the abstract and Results) --- rests on the fabricated potencies and
is withdrawn pending either removal or re-derivation on a recorded, real panel
(\texttt{data/mmp1\_panel\_pubchem.csv}, 121 compounds with per-row PubChem CID, assay AID and retrieval
timestamp). The reproducibility content (replicate dispersion, the ReplicaExchangeSampler deadlock, the
NaN-warmup fix, and the natural-product cross-method section) is a statement about whether a computation
repeats on a fixed input and is unaffected. The decision to restate this work as a
reproducibility-and-limitations paper that claims no accuracy, or to re-run the accuracy axis on the real
panel, is the author's and has not been made; until then this draft is not for submission or deposit.
Scope and correction map: \texttt{preprints/\_metadata/FABRICATED\_PANEL\_SCOPE\_2026\_07\_16.md}.
...
The experimental binding free energies used for the accuracy comparisons in
this draft were taken from the calibration panel file
(\texttt{data/chembl\_mmp1\_calibration.csv}) as
$\dGexp = RT \ln \mathrm{IC}_{50,\text{M}}$ at $T = \SI{310}{\kelvin}$
($RT = \SI{0.616}{\kcalmol}$), assuming $K_i \approx \mathrm{IC}_{50}$. A
primary-source audit (2026-07-16) subsequently established that this file's
potency values are fabricated and do not correspond to real compounds (see the
provenance note at the head of this document); the $\dGexp$ values are therefore
not verified experimental references, and every accuracy comparison against them
is withdrawn pending re-derivation on a recorded, real panel.
