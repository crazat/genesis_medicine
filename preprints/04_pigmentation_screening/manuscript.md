...
> **Correction (2026-07-21) — calibration-anchor attribution and metric.** The abstract described a "ChEMBL pIC50 calibration anchor carried over from the parallel MMP-1 calibration set (Pearson R = -0.453, n = 93)" and offered it as establishing that Boltz-2 `affinity_probability_binary` is a relative-ranking predictor. Two defects are corrected here. (1) **Attribution**: the -0.453 figure does not come from the MMP-1 calibration panel withdrawn in 2026-07 for fabricated annotations, nor from preprint #8; it comes from the real 93-compound ChEMBL cohort of preprint #18 (`pilot/cpu_meaningful/chembl_boltz2_calibration.csv`, carrying per-record ChEMBL document identifiers). The figure itself is sound and reproduces on recomputation. (2) **Metric**: -0.453 was measured on Boltz-2 `affinity_pred_value`, whereas this screen ranks on `affinity_prob_binary`. On the same 93 compounds `affinity_prob_binary` correlates with pIC50 at Pearson +0.18 / Spearman +0.05 — it carries essentially no potency signal — so the anchor never certified the metric used here. The rankings reported in this work are therefore **relative and uncalibrated**; no ranking value changes. Full scope of the underlying finding: `preprints/_metadata/FABRICATED_PANEL_SCOPE_2026_07_16.md`; citation audit: `preprints/_metadata/DANGLING_CITATION_AUDIT_2026_07_21.md`.
>
